...
# @param user_id [Integer], valid if friend_type != none
  ...
  def self.get_notation(user)
    user.friend_news.select('news.*, user_news_ranks.updated_at AS event_time').where(:news=>{:special_flag => false}).order('user_news_ranks.updated_at DESC').paginate :page => page, :per_page => @loading_news_num
  end
