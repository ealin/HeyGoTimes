    ...
    case friend_type
      ...
      when :mine
        if (type == 'latest')
          ...
          user.my_news_by_rank.select('news.*, user_news_ranks.*').where(:news=>{:special_flag => false}).order('user_news_ranks.rank DESC, news.created_at DESC')
        end
      ...
      when :friend
        if (type == 'latest')
          ...
          user.friend_news_by_rank.select('news.*, user_news_ranks.*').where(:news=>{:special_flag => false}).order('user_news_ranks.rank DESC, news.created_at DESC')
        end
      ...
      when :both
        if (type == 'latest')
          ...
          user.both_news.select('news.*, user_news_ranks.*').where(:news=>{:special_flag => false}).order('user_news_ranks.rank DESC, news.created_at DESC')
        end
