  ...
  def self.find_by_tags(type, friend_type, user_id, user_areas, user_tags)
    ...
    case friend_type
      ...
      when :both
        ...
        if (type == 'latest')
          user.both_news.joins(:user_news_ranks, :areas, :tags).where(:news=>{:special_flag => false}, :areas => {:name => user_areas}, :tags => {:name => user_tags}).order('user_news_ranks.rank DESC, news.created_at DESC')
        else
          user.both_news.joins(:user_news_ranks, :areas, :tags).where(:news=>{:special_flag => false}, :areas => {:name => user_areas}, :tags => {:name => user_tags}).order('news.created_at DESC')
        end
